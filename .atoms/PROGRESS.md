...
- 2026-09-23 **更新日志「没更新」定位并修复**：`/changelog` 数据源 `src/data/changelog.ts`（`v0.10.0` 已存在）与源文件渲染均正常（实测渲染 `total: 65 · versions: 10 · latest: 2026-09-23`），问题出在**生产构建产物陈旧**——`dist/assets/index-CncBEn7i.js` 生成于 `19:49:49`，而 `src/data/changelog.ts` 最后修改于 `19:53:52`，因此 `dist` 中既无 `v0.10.0` 也无「缺陷提交与自动修复」，页面上只显示到阶段四的 `v0.9.0`。触发原因是上一轮文档轮次按「纯文档不做构建」的约定跳过了构建，而发布/部署读取的是 `dist`。已重新执行 `pnpm run lint && pnpm run build`（退出码 0，预渲染 `/` 与 `/blog/`），产物哈希由 `index-CncBEn7i.js` 更新为 `index-COSMyCcd.js`，校验 `dist` 内已含 `v0.10.0` 与「缺陷提交与自动修复」，`dist/index.html` 引用同步指向新哈希。约定补充：**凡改动 `src/data/changelog.ts` 或任何前端源码的轮次，必须重新构建，不能按纯文档轮次跳过**。

- 2026-09-23 阶段五文档一致性复核（纯文档轮次，不做构建）：以真实路由与前端封送为准，修正三处与代码不符的接口路径——`docs/frontend.md` 与 `docs/plan.md` 把缺陷面板 / 创建缺陷由 `/api/v1/bugs/projects/{id}/panel`、`/api/v1/bugs/projects/{id}/bugs` 更正为真实的 `GET|POST /api/v1/bugs/projects/{id}`，`docs/backend.md` 把并不存在的 `GET /projects/{project_id}/fixes` 删除、只保留真实的 `GET /{bug_id}/fixes`；`docs/frontend.md` 补齐被遗漏的阶段四 / 五超时常量（`TEST_TIMEOUT_MS = 120_000`、`BUG_TIMEOUT_MS = 240_000` 及与其对应的后端 `180s` 模型硬超时）。核对通过：`routers/bug_fix.py` 六个端点与 `lib/projects.ts` 六个封装逐条对应；`docs/changelog.md` 的 10 个版本 + 65 条条目与 `src/data/changelog.ts` 的 10 个 `version` 完全一致，与根 `README.md`「10 个版本、65 条」表述相符。
- 2026-09-23 阶段五文档同步（纯文档轮次，不做构建）：发布记录追加 `v0.10.0`（缺陷提交与自动修复，6 新增 / 4 优化 / 2 修复），`docs/changelog.md` 与数据源逐条同步，当前共 10 个版本、65 条条目（两侧计数一致）；`docs/mission.md` 把修复层标记为已落地，`docs/plan.md` 补阶段五数据模型、模块划分、实施状态与交付清单，`docs/backend.md` 补缺陷数据模型、`/api/v1/bugs` 接口清单、自动修复链路与 `verify_bug_fix.py` 验证项，`docs/frontend.md` 与 `app/frontend/README.md` 补阶段五面板、接口契约、缓存刷新与修复后复测说明，根 `README.md` 补阶段五进度段、验证脚本与文档索引，`.wiki.md` 覆盖阶段五模块与文件清单；同时清除 `README.md`、`docs/plan.md`、`docs/frontend.md`、`app/frontend/README.md` 中残留的旧版本号与旧条目数表述，统一口径为「修复成败由新产物上的真实复测结论决定」。
