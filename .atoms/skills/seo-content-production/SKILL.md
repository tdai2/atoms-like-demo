...
WebSearchEngine usage: call `WebSearchEngine.run` with `query` set to a search combining
the product type, keywords, and either "SEO strategy" or "content ideas"; set `flag`
to `deep search`, then use the returned findings before continuing.
