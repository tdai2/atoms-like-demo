...
│   └── backend/      # Atoms Cloud 后端（认证、项目、任务、版本、配额、测试、缺陷与修复）
...
阶段二同时补齐了账号体验与发布记录：顶栏账号区在登录后显示账号邮箱与「退出登录」，未登录显示「登录 / 免费开始」；首页页脚不再出现与账号状态无关的登录入口。
...
阶段五**缺陷提交与自动修复已完成**：`bugs`、`bug_fix_logs` 两张表按账号隔离建表，可提交标题、描述、严重级别、复现步骤并关联已有测试用例，支持编辑、关闭、重开与删除。自动修复由真实模型结合方案、缺陷描述与当前产物改写代码，结果作为**新版本**上传（不覆盖原产物），随后在新产物上重跑关联用例给出复测结论；**修复是否成功由新产物上的真实复测结论决定**，模型自述不作依据。修复期间缺陷置为「修复中」并拒绝并发修复；模型或对象存储失败时如实落一条失败修复记录，缺陷停在可见失败态且不改动项目版本与产物指针。项目详情页已接入缺陷面板，覆盖加载、无产物、空数据与错误态；删除项目会同步清理缺陷与修复记录。

`/changelog` 目前共 10 个版本、65 条变更条目，最新为 `v0.10.0`（缺陷提交与自动修复），上一版为 `v0.9.0`（测试用例与执行闭环）。
...
python verify_bug_fix.py          # 阶段五：缺陷提交、修复失败如实落库、复测与清理
```
...
| [docs/changelog.md](docs/changelog.md) | 平台发布记录：`v0.10.0` 至 `v0.1.0` 的版本、日期、摘要、标签与变更条目 |
