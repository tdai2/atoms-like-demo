...
- **变更必须留痕**：每次生成与修改都写入项目变更日志，用户能回看「这一版改了什么」，而不是只看到最终结果。
...
每个阶段结束时都向项目变更日志追加一条记录（失败也记录，并标注失败阶段与原因），因此生成历史可以被完整回看。
...
| `change_logs` | `id` `project_id` `version_id` `action` `summary` `detail` `source` | 项目变更日志，一次生成或修改一条，只读 |
...
| 变更日志 | 按阶段追加项目变更记录，提供分页查询与单条明细 | `GET /projects/{id}/changelog`、`GET /projects/{id}/changelog/{log_id}` |

### 2.3 模块依赖方向

```
体验层 → 接口层 → { 鉴权, 项目管理, 生成编排, 变更日志 }
                      生成编排 → AI 网关 → 模型
                      生成编排 → 产物存储
                      生成编排 → 配额
                      生成编排 → 变更日志（每阶段结束自动追加）
...
依赖单向向下，禁止反向调用；AI 网关只被生成编排使用，保证模型替换不外溢；变更日志只由生成编排写入、由项目管理读取，用户不可直接修改。
...
| 变更日志谁写 | 生成编排在阶段结束时自动追加 | 人工补写必然漏记，日志的可信度来自流水线自己生成 |
| 变更日志是否可编辑 | 否，只读 | 一旦允许改写，回溯与版本对比就失去意义 |
...
| 四 | 对话式增量修改、版本回溯、项目变更日志 | 每次修改都留下一版可读的变更记录         |
...
### 变更日志（Change Log）功能范围

| 项 | 说明 |
|----|------|
| 目的 | 让用户看清「这一版改了什么」，而不是只看到最终结果；对应 `mission.md` 的「过程透明优于结果惊喜」 |
| 记录时机 | 一次生成或一次增量修改产生一条记录；阶段失败同样记录，并标注失败阶段与原因 |
| 记录内容 | 版本号、触发方式（首次生成 / 对话式修改）、变更类型、面向用户的一句话变更摘要、涉及文件与影响范围、关联任务与预览地址 |
| 摘要来源 | 由生成编排在「生成方案」阶段产出，随代码产物一并落库，不依赖人工补写（沿用 `claude-opus-5`） |
| 用户可见性 | 项目详情页的变更日志列表，按时间倒序，可展开查看单次变更明细并跳转到版本对比 |
| 只读约束 | 日志由流水线写入，用户只能查看与回溯，不可编辑或删除 |
| 对外接口 | `GET /projects/{id}/changelog`（分页列表）、`GET /projects/{id}/changelog/{log_id}`（单条明细） |
| 与版本链的关系 | 一条变更日志对应一个 `project_versions` 版本，回溯到某一版等价于选中该条日志 |
| 前端落地 | 待阶段二接入后端后实现；阶段一的 `/changelog` 占位页是平台自身的更新日志，与此处的项目变更日志不是同一对象 |