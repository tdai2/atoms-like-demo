...
```
用户意图 → 需求解析 → 方案规划 → 代码生成 → 构建校验 → 发布预览 → 可访问的应用
              ↑                                                    │
              └──────────────── 对话式增量修改 ←───────────────────┘
```
...
| 预览窗形态 | 当前真实组件渲染，接入后改 iframe | 演示阶段更可信，接入后自然过渡到真实部署 |
...
| 二 | 激活后端，落地账号、项目表、生成接口 | 登录后能看到自己的项目列表 |
| 三 | 接入真实模型生成，替换预设匹配 | 输入任意需求产出可访问链接 |
| 四 | 对话式增量修改、版本回溯 | 追加需求只改动对应部分 |
| 五 | 自定义域名、团队协作与权限 | 项目可对外正式发布 |
