...
2. Database First (if you need to create tables)
   ...
   - Before expanding data-dependent business pages, complete the Data Contract Checkpoint below.
...
1. Database Changes (if needed)
   ...
   - If the contract changes, complete the Data Contract Checkpoint before changing its dependent pages.
2. New features (if needed)
   - Generate the remaining or new features using BackendManager.create_function

### Data Contract Checkpoint

- Use the current application's actual schema, or the authoritative result of successful table creation. If that result lacks necessary fields, query the relevant schema with `BackendManager.get_schemas` before implementing dependent pages; do not query again per page or reread generated files for reassurance.
- Confirm the fields the feature uses: names, types, required/optional values, relationships, and the documented SDK/API request and response shapes. Derive shared frontend types, data access, forms, and submitted payloads from this same contract; do not independently invent or rename persisted fields in pages.
- Reuse this contract until it changes or a concrete mismatch appears. When a requirement needs a change, update the contract first, then synchronize affected consumers. No separate contract document or full-schema dump is required.
