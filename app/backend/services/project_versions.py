...
# ------------------ Service Layer ------------------
class Project_versionsService:
    """Service layer for Project_versions operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Project_versions]:
        """Create a new project_versions"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Project_versions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created project_versions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating project_versions: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for project_versions {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Project_versions]:
        """Get project_versions by ID (user can only see their own records)"""
        try:
            query = select(Project_versions).where(Project_versions.id == obj_id)
            if user_id:
                query = query.where(Project_versions.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching project_versions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of project_versionss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Project_versions.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Project_versions, field):
                        column = getattr(Project_versions, field)
            ...
            stmt = select(Project_versions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Project_versions, field_name):
                        stmt = stmt.order_by(getattr(Project_versions, field_name).desc())
                else:
                    if hasattr(Project_versions, sort):
                        stmt = stmt.order_by(getattr(Project_versions, sort))
            else:
                stmt = stmt.order_by(Project_versions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Project_versions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching project_versions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Project_versions]:
        """Update project_versions (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Project_versions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated project_versions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating project_versions {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete project_versions (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Project_versions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted project_versions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting project_versions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Project_versions]:
        """Get project_versions by any field"""
        try:
            if not hasattr(Project_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Project_versions")
            column = getattr(Project_versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Project_versions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching project_versions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Project_versions]:
        """Get list of project_versionss filtered by field"""
        try:
            if not hasattr(Project_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Project_versions")
            column = getattr(Project_versions, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Project_versions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching project_versionss by {field_name}: {str(e)}")
            raise