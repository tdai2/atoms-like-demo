...
# ------------------ Service Layer ------------------
class BugsService:
    """Service layer for Bugs operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bugs]:
        """Create a new bugs"""
        try:
            if user_id:
            ...
            logger.info(f"Created bugs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating bugs: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for bugs {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Bugs]:
        """Get bugs by ID (user can only see their own records)"""
        try:
            query = select(Bugs).where(Bugs.id == obj_id)
            if user_id:
                query = query.where(Bugs.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bugs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of bugss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Bugs.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Bugs, field):
                        column = getattr(Bugs, field)
            ...
            stmt = select(Bugs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Bugs, field_name):
                        stmt = stmt.order_by(getattr(Bugs, field_name).desc())
                else:
                    if hasattr(Bugs, sort):
                        stmt = stmt.order_by(getattr(Bugs, sort))
            else:
                stmt = stmt.order_by(Bugs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Bugs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching bugs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bugs]:
        """Update bugs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bugs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated bugs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating bugs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete bugs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bugs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted bugs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting bugs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Bugs]:
        """Get bugs by any field"""
        try:
            if not hasattr(Bugs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bugs")
            column = getattr(Bugs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Bugs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bugs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Bugs]:
        """Get list of bugss filtered by field"""
        try:
            if not hasattr(Bugs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bugs")
            column = getattr(Bugs, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Bugs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching bugss by {field_name}: {str(e)}")
            raise