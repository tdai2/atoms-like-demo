...
# ------------------ Service Layer ------------------
class Usage_quotasService:
    """Service layer for Usage_quotas operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Usage_quotas]:
        """Create a new usage_quotas"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Usage_quotas(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created usage_quotas with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating usage_quotas: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for usage_quotas {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Usage_quotas]:
        """Get usage_quotas by ID (user can only see their own records)"""
        try:
            query = select(Usage_quotas).where(Usage_quotas.id == obj_id)
            if user_id:
                query = query.where(Usage_quotas.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching usage_quotas {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of usage_quotass (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Usage_quotas.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Usage_quotas, field):
                        column = getattr(Usage_quotas, field)
            ...
            stmt = select(Usage_quotas, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Usage_quotas, field_name):
                        stmt = stmt.order_by(getattr(Usage_quotas, field_name).desc())
                else:
                    if hasattr(Usage_quotas, sort):
                        stmt = stmt.order_by(getattr(Usage_quotas, sort))
            else:
                stmt = stmt.order_by(Usage_quotas.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Usage_quotas)
                for condition in conditions:
            ...
            logger.error(f"Error fetching usage_quotas list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Usage_quotas]:
        """Update usage_quotas (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Usage_quotas {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated usage_quotas {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating usage_quotas {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete usage_quotas (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Usage_quotas {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted usage_quotas {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting usage_quotas {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Usage_quotas]:
        """Get usage_quotas by any field"""
        try:
            if not hasattr(Usage_quotas, field_name):
                raise ValueError(f"Field {field_name} does not exist on Usage_quotas")
            column = getattr(Usage_quotas, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Usage_quotas).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching usage_quotas by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Usage_quotas]:
        """Get list of usage_quotass filtered by field"""
        try:
            if not hasattr(Usage_quotas, field_name):
                raise ValueError(f"Field {field_name} does not exist on Usage_quotas")
            column = getattr(Usage_quotas, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Usage_quotas.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching usage_quotass by {field_name}: {str(e)}")
            raise