...
# ------------------ Service Layer ------------------
class Test_runsService:
    """Service layer for Test_runs operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Test_runs]:
        ...
        try:
            ...
            logger.info(f"Created test_runs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating test_runs: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for test_runs {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Test_runs]:
        """Get test_runs by ID (user can only see their own records)"""
        try:
            query = select(Test_runs).where(Test_runs.id == obj_id)
            if user_id:
                query = query.where(Test_runs.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching test_runs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of test_runss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Test_runs.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Test_runs, field):
                        column = getattr(Test_runs, field)
            ...
            stmt = select(Test_runs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Test_runs, field_name):
                        stmt = stmt.order_by(getattr(Test_runs, field_name).desc())
                else:
                    if hasattr(Test_runs, sort):
                        stmt = stmt.order_by(getattr(Test_runs, sort))
            else:
                stmt = stmt.order_by(Test_runs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Test_runs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching test_runs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Test_runs]:
        """Update test_runs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Test_runs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated test_runs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating test_runs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete test_runs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Test_runs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted test_runs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting test_runs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Test_runs]:
        """Get test_runs by any field"""
        try:
            if not hasattr(Test_runs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Test_runs")
            column = getattr(Test_runs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Test_runs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching test_runs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Test_runs]:
        """Get list of test_runss filtered by field"""
        try:
            if not hasattr(Test_runs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Test_runs")
            column = getattr(Test_runs, field_name)
            ...
            logger.error(f"Error fetching test_runss by {field_name}: {str(e)}")
            raise