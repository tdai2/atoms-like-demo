...
# ------------------ Service Layer ------------------
class Bug_fix_logsService:
    """Service layer for Bug_fix_logs operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bug_fix_logs]:
        """Create a new bug_fix_logs"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Bug_fix_logs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created bug_fix_logs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating bug_fix_logs: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for bug_fix_logs {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Bug_fix_logs]:
        """Get bug_fix_logs by ID (user can only see their own records)"""
        try:
            query = select(Bug_fix_logs).where(Bug_fix_logs.id == obj_id)
            if user_id:
                query = query.where(Bug_fix_logs.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bug_fix_logs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of bug_fix_logss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Bug_fix_logs.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Bug_fix_logs, field):
                        column = getattr(Bug_fix_logs, field)
            ...
            stmt = select(Bug_fix_logs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Bug_fix_logs, field_name):
                        stmt = stmt.order_by(getattr(Bug_fix_logs, field_name).desc())
                else:
                    if hasattr(Bug_fix_logs, sort):
                        stmt = stmt.order_by(getattr(Bug_fix_logs, sort))
            else:
                stmt = stmt.order_by(Bug_fix_logs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Bug_fix_logs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching bug_fix_logs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bug_fix_logs]:
        """Update bug_fix_logs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bug_fix_logs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated bug_fix_logs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating bug_fix_logs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete bug_fix_logs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bug_fix_logs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted bug_fix_logs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting bug_fix_logs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Bug_fix_logs]:
        """Get bug_fix_logs by any field"""
        try:
            if not hasattr(Bug_fix_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bug_fix_logs")
            column = getattr(Bug_fix_logs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Bug_fix_logs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bug_fix_logs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Bug_fix_logs]:
        """Get list of bug_fix_logss filtered by field"""
        try:
            if not hasattr(Bug_fix_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bug_fix_logs")
            column = getattr(Bug_fix_logs, field_name)
            ...
            logger.error(f"Error fetching bug_fix_logss by {field_name}: {str(e)}")
            raise