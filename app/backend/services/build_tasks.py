...
# ------------------ Service Layer ------------------
class Build_tasksService:
    """Service layer for Build_tasks operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Build_tasks]:
        """Create a new build_tasks"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Build_tasks(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created build_tasks with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating build_tasks: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for build_tasks {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Build_tasks]:
        """Get build_tasks by ID (user can only see their own records)"""
        try:
            query = select(Build_tasks).where(Build_tasks.id == obj_id)
            if user_id:
                query = query.where(Build_tasks.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching build_tasks {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of build_taskss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Build_tasks.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Build_tasks, field):
                        column = getattr(Build_tasks, field)
            ...
            stmt = select(Build_tasks, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Build_tasks, field_name):
                        stmt = stmt.order_by(getattr(Build_tasks, field_name).desc())
                else:
                    if hasattr(Build_tasks, sort):
                        stmt = stmt.order_by(getattr(Build_tasks, sort))
            else:
                stmt = stmt.order_by(Build_tasks.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Build_tasks)
                for condition in conditions:
            ...
            logger.error(f"Error fetching build_tasks list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Build_tasks]:
        """Update build_tasks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Build_tasks {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated build_tasks {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating build_tasks {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete build_tasks (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Build_tasks {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted build_tasks {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting build_tasks {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Build_tasks]:
        """Get build_tasks by any field"""
        try:
            if not hasattr(Build_tasks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Build_tasks")
            column = getattr(Build_tasks, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Build_tasks).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching build_tasks by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Build_tasks]:
        """Get list of build_taskss filtered by field"""
        try:
            if not hasattr(Build_tasks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Build_tasks")
            column = getattr(Build_tasks, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Build_tasks.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching build_taskss by {field_name}: {str(e)}")
            raise