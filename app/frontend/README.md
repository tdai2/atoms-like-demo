...
- **阶段五（缺陷修复闭环已完成）**：项目详情页接入缺陷面板，可提交缺陷并关联用例、触发自动修复，修复产出新版本并在新产物上复测，修复历史与结论可回溯。
...
| `/projects/:id` | 项目详情：六阶段快照、方案、测试报告、版本、失败重试、测试面板与缺陷修复面板 | 已实现 |
...
| `src/components/TestSuitePanel.tsx` | 阶段四测试面板：用例生成、手动增删改、执行、结果统计、运行历史切换与「修复后复测」标注 |
...
- 发布记录集中在 `src/data/changelog.ts`：新增版本时在数组顶部追加一条 `Release`，页面会自动置顶并标记 `LATEST`，类型筛选计数与版本跳转同步更新；当前最新为 `v0.10.0`（缺陷提交与自动修复）。
...
项目详情页的缺陷面板（`src/components/BugPanel.tsx`）把「发现问题 → 记录 → 修掉 → 复测」串成闭环：

1. 提交缺陷：填写标题、描述、严重级别与复现步骤，可关联已有测试用例；没有产物的项目会被拒绝提交。
2. 自动修复：`POST /api/v1/bugs/{bugId}/fix` 由真实模型结合方案、缺陷描述与当前产物改写代码，结果作为**新版本**上传，不覆盖原产物。
3. 修复后复测：修复成功后自动在新产物上重跑关联用例，复测结论与运行记录一并落库，测试面板中以「修复后复测」标注。
4. 修复历史：每次尝试的源 / 目标版本、结论与失败原因可展开查看。
5. 状态与并发：缺陷在修复期间置为「修复中」并拒绝并发修复；模型或存储失败时如实落一条失败修复记录，缺陷停在可见失败态，且不改动项目版本与产物指针。
6. 状态覆盖：加载中、无产物、空数据与错误态均有明确反馈；创建、编辑、关闭、重开与删除缺陷都可直接操作。
